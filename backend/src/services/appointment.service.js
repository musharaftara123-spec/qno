...
// Atomically increments (or creates) the counter for one doctor+day slot
// and returns the new count as the token number. Using findOneAndUpdate
// with $inc means two simultaneous bookings can never read-then-write the
// same stale count — the DB serializes it, unlike the old localStorage
// version which had no such guarantee.
async function nextTokenForSlot(doctorId, day, bookedBy) {
  const isOnlineBooking = bookedBy !== 'receptionist'

  if (isOnlineBooking) {
    // Pre-check so we can return a clean error instead of incrementing
    // past the cap and having to roll back.
    const existing = await SlotCounter.findOne({ doctor: doctorId, day })
    if (existing && existing.count >= MAX_ONLINE_BOOKINGS_PER_SLOT) {
      throw new ApiError(
        409,
...
export async function createAppointment({
  ...
  // Throws SLOT_FULL (409) if this is an online booking and the slot is
  // already at the cap — before we generate a patientId or write anything.
  const tokenNumber = await nextTokenForSlot(doctor._id, day, bookedBy)
